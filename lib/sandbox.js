...
return ((vm, host) => {
		...
		EXTENSIONS["." + ext] = (module, filename, dirname) => {
			if (vm.options.require.context !== 'sandbox') {
				...
				try {
					var contents;
					// Load module
					if(vm.options.require.callbackRegex && 
						vm.options.require.callbackRegex.exec(filename) &&
						vm.options.require.callback && 
						typeof(vm.options.require.callback) === 'function') {
                            // callbackRegex matches now call the callback function for dynamic 
                            // module loading from strings
							filename = filename.replace(vm.options.require.callbackRegex, ""); 
							contents = vm.options.require.callback(filename);
					} else {
	...
	const _prepareRequire = function(current_dirname) {
		const _require = function(modulename) {
			...
			if (vm.options.require.callbackRegex && vm.options.require.callbackRegex.exec(modulename)) {
				// skip the filename validation and extension check
				var filename = modulename;
                var extJs = ".js";
			} else if (/^(\.|\.\/|\.\.\/)/.exec(modulename)) {
			...
			const extname = extJs? extJs: pa.extname(filename);
